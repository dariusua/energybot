...
def send(message: types.Message):
    if message.from_user.id == 880691612 or message_from_user.id == 720509891:
        ...
        result_bagged_users = cursor.execute("SELECT COUNT(*) FROM database WHERE night != 1 AND night != 0 AND maybe != 1 AND maybe != 0 AND time_to != 10 AND time_to != 30 AND time_to != 60").fetchone()
