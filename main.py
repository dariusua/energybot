...
def send(message: types.Message):
    if message.from_user.id == 880691612 or message.from_user.id == 720509891:
        ...
        result_night_not = cursor.execute("SELECT COUNT(*) FROM database WHERE night = 0").fetchone()
        result_maybe = cursor.execute("SELECT COUNT(*) FROM database WHERE maybe = 1").fetchone()
        result_maybe_not = cursor.execute("SELECT COUNT(*) FROM database WHERE maybe = 0").fetchone()
